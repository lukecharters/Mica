...
These six are **not** generated by the script. They are captures of the running
app, taken with the `drive-mica-on-screen` skill, and they go stale on any UI
change. Retake them by hand.

| Image | Page | State to set up |
|---|---|---|
| `window-anatomy.png` | `The-Mica-Window`, `Getting-Started` | Advanced controls off, `star.fill`, badge off. Pane labels are drawn onto the PNG. |
| `symbol-browser.png` | `The-Mica-Window`, `Getting-Started` | Symbol browser open, `star` typed in Search, `star.fill` selected. |
| `sidebar-eyes.png` | `The-Mica-Window`, both settings pages | Advanced controls on, badge active, icon background hidden, so the Icon eye is mixed. |
| `advanced-off.png` | Both settings pages | Icon group, advanced controls off. Same crop as `advanced-on.png`. |
| `advanced-on.png` | Both settings pages | Icon Foreground layer, advanced controls on. Same crop as `advanced-off.png`. |
| `settings-importing.png` | `Icon-Settings` | Settings ▸ Importing, both toggles on. |

Three things to know before retaking them:

- **Capture the screen region, not the window.** `screencapture -l <windowID>`
  and `peekaboo image --retina` both return stale widgets drawn in the sidebar's
  empty area that are not on screen and are absent from the accessibility tree.
  `screencapture -R x,y,w,h` over the window's bounds is clean.
- **A region capture picks up anything drawn over the window**, so keep the
  window clear of the Dock and of notification banners, and check every capture
  before committing it. Put the window low enough to miss the banners and high
  enough to miss the Dock.
- **Light appearance, and no personal content.** No Dock, no file paths, no
  badge counts.
